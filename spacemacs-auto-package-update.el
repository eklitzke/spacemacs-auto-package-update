...
(defvar spacemacs-auto-package/update-file
  (expand-file-name "~/.emacs.d/private/last-package-update")
  "The file to track update times using.")

(defun spacemacs-auto-package-update/do-update ()
  ...
  (with-temp-file spacemacs-auto-package-update-file (save-buffer)))

(defun spacemacs-auto-package-update/check-and-apply-update ()
  "Apply an update if the file is too old or doesn't exist."
  (when (or (not (file-exists-p spacemacs-auto-package/update-file))
            (let* ((now (time-convert nil 'integer))
                   (last-seen (time-convert
                               (file-attribute-modification-time
                                (file-attributes spacemacs-auto-package/update-file)) 'integer))
                   (delta (- now last-seen)))
              (>= delta spacemacs-auto-package/update-age)))
    (spacemacs-auto-package-update/do-update)))
